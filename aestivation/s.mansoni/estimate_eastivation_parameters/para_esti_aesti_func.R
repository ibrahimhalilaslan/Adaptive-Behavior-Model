...
# Choose only portion of combination due to the computation cost
...
# This function convert MPB into prevalence value
...
# Set how pricely to take step while solving ode
step_size <- 0.1
...
# Create vectors and list to record the outcome
square_error <- c()
prevalence_percentage_m <- list()
...
for (m in 1:nrow(combinations)){
  ...
  aest_temp <- combinations[m, 1]
  
  # set the rate of snails move to the estivation stages
  aest_rate <- combinations[m, 2]
  
  # Set the steepness of estivation function for hot temperature 
  steepness <-  combinations[m, 3]
...
out_come_season_mpb[out_come_season_mpb < .8] <- 0

# record the out put of model simulation
prevalence_percentage_m[[m]] <- wormPrevalenceSm(out_come_season_mpb)
prevalence_percentage <- wormPrevalenceSm(out_come_season_mpb)*100
square_error[m] <- sum((prevalence_percentage-new_gntd_data)^2)
}

# Save the environment
save.image(file = "my_environment_10.RData")
