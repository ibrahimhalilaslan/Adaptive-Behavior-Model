## This code is to plot generate the data frame for constant temperature and 
# seasonal temperature with adaptive behavior model
...
# load the data for the simulation running with constant temperature 
load(file ="august_2.RData")

#upload data for the simulation running with 
#seasonal temperature and adaptive behavior model
prevalence_africa <- read.csv(file = 'Sm_simulation_africa_time_series_mpb.csv')

#Replace the prevalence value with zero when it is smaller than the threshold value 
index <- which(prevalence_africa$MPB < 0.74)
prevalence_africa$MPB[index] <- 0


prevalence_africa$constant_temp_simulation <- numeric(length(prevalence_africa$Prevalence))

# Find the prevalence value with simulation of constant temperature  
...
#define function to covert the MPB into the prevalence
